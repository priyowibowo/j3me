 ...
 */
class ApinorwegianControllerApinorwegian extends JControllerForm
{

  public function import(){
    require_once(JPATH_ROOT.'/administrator/components/com_apinorwegian/helpers/apinorwegian.php');
    $data = ApinorwegianHelper::APIqueryDestinations('RejseEksperterneDest', 'EN');
    $model = $this->getModel();
    $model->import($data);
    $this->setRedirect('index.php?option=com_apinorwegian&view=apinorwegians');
  }
  ...
  public function flighsearch(){
    ...
    $hasil = ApinorwegianHelper::APIflightsearchprivateclass(0, 24,'RejseEksperterne', $data['currency'], $data['origin'], $data['destination'], $tanggal, $data['classf']);
